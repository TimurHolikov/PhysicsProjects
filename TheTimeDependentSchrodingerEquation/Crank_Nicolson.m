...
%% Velocity and width
% Sometimes works, but not sures

% v_idx = find(abs(psi(Nt, :)).^2 == max(abs(psi(Nt, :)).^2));
% v = (x(v_idx)-x(x_0))/t(Nt); 

% d = zeros(1, Nt);
width = zeros(1, Nt);
width_function = @(sigma, t, m) sigma * sqrt(1 + (t / (2 * m * sigma^2))^2);
...
for ts = 1:9:Nt
    % d(ts) = calculate_width(x, abs(psi(:, ts)).^2);
    ...
    plot(x, real(psi(:, ts)))
        ...
        % plot(x, real(psi(:,1)), "b")
        % hold off
        % Uncomment to show initial state on the plot
    grid on
    box on
    xlabel('x')
    ylabel('\psi_{real}')
    ...
    xlim([0,400])
    % title(['Time Step: ' num2str(ts) ', Velocity: ' num2str(v) ', Expected velocity: ' num2str(q/m) ...
    %     ', Width: ' num2str(d(ts)) ', Expected width: ' num2str(width(ts))])
    title(['Time Step: ' num2str(ts) ', Expected velocity: ' num2str(q/m) ', Expected width: ' num2str(width(ts))])

    subplot(2, 1, 2)
    plot(x, abs(psi(:, ts)).^2,"r")
    % hold on
    % plot(x, abs(psi(:, 1)).^2, "r")
    % hold off
    grid on
    box on
    xlabel('x')
...
function [psi, Omega] = crank_nicolson_solve(Nx, dx, Nt, dt, V, m, x, sigma, q, x_0, h)
    ...
    disp(['Integral of |psi|^2:', num2str(integral_value)]);
end
...
function psi0 = initial_psi0(x, sigma, q, x_0, h)
    ...
    disp(['Integral of |psi0|^2:', num2str(integral_value)]);
end

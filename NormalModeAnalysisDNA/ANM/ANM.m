...
animate_modeset(R0,N,phi_small,omega_small,t,15, ...
    xlim_,ylim_,zlim_,'Small Mode','small_eigenmodes_animation.gif');
animate_modeset(R0,N,phi_large,omega_large,t,8, ...
    xlim_,ylim_,zlim_,'Large Mode','large_eigenmodes_animation.gif');

%% Functions
function animate_modeset(R0,N,phi_set,omega_set,t,scale,xlim_,ylim_,zlim_,title_prefix,gif_name)
    function plot_dna(R,R0,N)
        plot3(R0(:,1),R0(:,2),R0(:,3),'-','Color',[0.8 0.8 0.8],'LineWidth',0.6);
    ...
    fprintf('Animating %s set...\n',title_prefix);

    % make window larger automatically
    screen = get(0,'ScreenSize');
    fig_w = min(0.9*screen(3), 1600);
    fig_h = min(0.5*screen(4), 600);
    figure('Color',[0.08 0.08 0.08],'Position',[50 100 fig_w fig_h]);

    for f = 1:length(t)
        for mode = 1:3
            phi = reshape(phi_set(:,mode),3,[]).';
            R = R0 + scale*sin(t(f))*phi;

            subplot(1,3,mode); cla; hold on;
            plot_dna(R,R0,N);
            title(sprintf('%s %d (ω = %.4f)',title_prefix,mode,omega_set(mode)), ...
        ...
        if f==1
            imwrite(im,map,gif_name,'gif','Loopcount',inf,'DelayTime',0.04);
        else
            imwrite(im,map,gif_name,'gif','WriteMode','append','DelayTime',0.04);
        end
    end
    fprintf('Saved %s\n',gif_name);
end
